...
from scipy import interpolate
    ...
    # st.set_page_config(page_title='แผ่นดินไหว มยผ',
...
def main():
    ...
    with col1:
        ...
        structure = st.radio(label='ประเภทโครงสร้าง',
                             options=structure_list, index=0, key='structure')
    with col2:
        ...
        damping = st.radio(label='ความหน่วง',
                           options=damping_list, index=0, key='damping')
    with col3:
        H = st.number_input(
    ...
    if not bkk:
        ...
        Ss = df_SsS1.loc[(df_SsS1['จังหวัด'] == province) & (
            df_SsS1['อำเภอ'] == district), 'Ss'].iloc[0]
        S1 = df_SsS1.loc[(df_SsS1['จังหวัด'] == province) & (
            df_SsS1['อำเภอ'] == district), 'S1'].iloc[0]
        ...
        soil_type = st.selectbox(label='ประเภทชั้นดิน', options=[
                                 'A', 'B', 'C', 'D', 'E', 'F'], index=0, key='soil_type')
    ...
    if bkk and damping == '2.5%':
        ...
        SDSx = df_bkkx.loc[(df_bkkx['zone'] == zone) & (
            df_bkkx['T'] == 0.2), 'Sa'].iloc[0]
        SD1x = df_bkkx.loc[(df_bkkx['zone'] == zone) & (
            df_bkkx['T'] == 1.0), 'Sa'].iloc[0]
    ...
    with col1:
        ...
        df_v_distribute = st.data_editor(
            df_v_distribute, num_rows="dynamic", key='df_v_distribute')
    with col2:
        st.write('**Output**')
