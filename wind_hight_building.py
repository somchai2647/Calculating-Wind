...
def main():
    ...
    with col1:
        Floor = st.number_input(
            label='จำนวนชั้น', min_value=1, max_value=24, value=3, step=1)
        Widthx = st.number_input(
            label=r'$ความกว้างตามแนวแกน \quad x$', min_value=0.0, value=3.0, step=0.1)
        Widthy = st.number_input(
            label=r'$ความกว้างตามแนวแกน \quad y$', min_value=0.0, value=3.0, step=0.1)
    ...
    with col2:
        ...
        with col1x:
            ...
            for i in range(Floor):
                Heigh = st.number_input(label=r'$ความสูงของชั้นที่ \quad %i$' % (
                    i+1), min_value=0.0, value=3.0, step=0.1, key=f"floor_{i}")
                H += Heigh
                with col2x:
                    if H > 80:
                        st.warning(
                            (r"$ความสูงของอาคารเกิน \quad 80 \quad m!$"))
                        break
                    Floor_list.append(H)
                    Floors.loc[i+1] = i+1
                    Wx = Wx + (H*Widthx)
                    ...
                    if Sum_Wx < H:
                        st.markdown('')
                        st.warning(
                            (r"$3 \quad เท่าของความกสว้างน้อยกว่าความสูง!$"))
                        break
                    Wy = Wy + (H*Widthy)
                    Sum_Wy = 3*Wy/H
                    if Sum_Wy < H:
    ...
    with col1:
        important_type = st.selectbox(
            label='ประเภทความสำคัญ', options=df_important['ประเภทความสำคัญ'])

    with col2:
        cal_type = st.selectbox(label='ประเภทการออกแบบ', options=[
                                'สภาวะจำกัดด้านกำลัง', 'สภาวะจำกัดด้านการใช้งาน'])

    with col3:
        Iw = float(
            df_important.loc[df_important['ประเภทความสำคัญ'] == important_type, cal_type])
        st.markdown('')
        st.markdown('')
        st.write(r'$I = %.2f$' % (Iw))
    ...
    with col1:
        area_group = st.selectbox(
            label='กลุ่มพื้นที่', options=df_wind_speed['กลุ่ม'])
    ...
    with col1:
        land_type = st.selectbox(
            label='สภาพภูมิประเทศ', options=['แบบ A', 'แบบ B'])
        Ceup_list = []
        if land_type == 'แบบ A':
            for i in Floor_list:
    ...
    with col1:
        Type_Building = st.selectbox(
            label='ประเภทอาคาร', options=df_Cpi['ประเภท'])
        col1x, col2x = st.columns(2)
        with col1x:
            Cpi_minus = float(
                df_Cpi.loc[df_Cpi['ประเภท'] == Type_Building, 'Cpi_minus'])
            st.write(r'$C_{pi}⁻ = %.2f$' % (Cpi_minus))
        with col2x:
            Cpi_plus = float(
                df_Cpi.loc[df_Cpi['ประเภท'] == Type_Building, 'Cpi_plus'])
    ...
    with tab1:
        ...
        Netx_pos['Net Pressure'] = Cg*q*Iw*(Netx_pos['Ce,up']*(Netx_pos['Cp,up'] - Netx_pos['Cpi']) + abs(
            Netx_pos['Ce,down']*(Netx_pos['Cp,down'] - Netx_pos['Cpi'])))
        ...
        with col2:
            ...
            chartnet_posx = alt.Chart(datanet_posx).mark_bar(size=Floor*12, color='white', stroke='red', strokeWidth=2).encode(
                x='Net Pressure', y=alt.Y('Floors', title='Floors'),).configure_mark(orient='horizontal').properties(height=Floor*61)
        ...
        Netx_neg['Net Pressure'] = Cg*q*Iw*(Netx_neg['Ce,up']*(Netx_neg['Cp,up'] - Netx_neg['Cpi']) + abs(
            Netx_neg['Ce,down']*(Netx_neg['Cp,down'] - Netx_neg['Cpi'])))
        ...
        with col2:
            ...
            chartnet_negx = alt.Chart(datanet_negx).mark_bar(size=Floor*12, color='white', stroke='red', strokeWidth=2).encode(
                x='Net Pressure', y=alt.Y('Floors', title='Floors'),).configure_mark(orient='horizontal').properties(height=Floor*61)
    ...
    with tab2:
        ...
        Nety_pos['Net Pressure'] = Cg*q*Iw*(Nety_pos['Ce,up']*(Nety_pos['Cp,up'] - Nety_pos['Cpi']) + abs(
            Nety_pos['Ce,down']*(Nety_pos['Cp,down'] - Nety_pos['Cpi'])))
        ...
        with col2:
            ...
            chartnet_posy = alt.Chart(datanet_posy).mark_bar(size=Floor*12, color='white', stroke='red', strokeWidth=2).encode(
                x='Net Pressure', y=alt.Y('Floors', title='Floors'),).configure_mark(orient='horizontal').properties(height=Floor*61)
        ...
        Nety_neg['Net Pressure'] = Cg*q*Iw*(Nety_neg['Ce,up']*(Nety_neg['Cp,up'] - Nety_neg['Cpi']) + abs(
            Nety_neg['Ce,down']*(Nety_neg['Cp,down'] - Nety_neg['Cpi'])))
        ...
        with col2:
            ...
            chartnet_negy = alt.Chart(datanet_negy).mark_bar(size=Floor*12, color='white', stroke='red', strokeWidth=2).encode(
                x='Net Pressure', y=alt.Y('Floors', title='Floors'),).configure_mark(orient='horizontal').properties(height=Floor*61)
